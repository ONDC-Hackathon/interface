...
function ColorandSize(props) {
    ...
    return (
        <>
            <StyledBox>
                ...
                <List className='flex flex-row justify-evenly'>
                    {colors.map((color, index) => (
                        ...
                        }} selected={(index == selectedColor)} onClick={() => setSelectedColor(index)}>
                            <div>
            ...
            <StyledBox>
                <Typography variant='h6' align='left' gutterBottom> Size: </Typography>
                <List className='flex flex-row justify-evenly'>
                    {size.map((size, index) => (
                        <ListItemButton selected={(index == selectedSize)} onClick={() => setSelectedSize(index)} >
                            <Typography align='left' component='div' sx={{ marginTop: '0.5rem', backgroundColor: '#F6F6F6', padding: '5px 10px' }} className='flex flex-row items-center' variant="subtitle1" gutterBottom>{size}</Typography>
                        </ListItemButton>
                    ))}
                </List>
            </StyledBox>
